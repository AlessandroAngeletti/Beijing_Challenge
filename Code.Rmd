...
regression_data <-  cleaning %>% 
  ...
  mutate(
    price_4_nights = case_when(guests_included < 2 ~ cleaning_fee + (4 * (price + extra_people)),
...
regression_data %>% 
  ...
  mutate(neighbourhood_simplified = neighbourhoodring$Ring[match(neighbourhood, neighbourhoodring$neighbourhood)]) 

regression_data
#  left_join(., neighbourhoodring, by = "neighbourhood", copy = TRUE)
  mutate(
    neighbourhood_simplified = case_when(
      neighbourhood = c("",) ~ "Zone 2"
      neighbourhood = c("",) ~ "Zone 3"
      neighbourhood = c("",) ~ "Zone 4"
      neighbourhood = c("",) ~ "Zone 5"
      TRUE ~ "Zone 6"
    )
  )
